      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {[
            ...
            <div key={idx} className="rounded-xl sm:rounded-2xl bg-white p-3.5 sm:p-5 shadow-sm border border-slate-200 flex flex-col justify-between hover:scale-[1.01] transition-all duration-200">
              <div className="flex justify-between items-start gap-1">
                <div>
                  <p className="text-[10px] sm:text-xs md:text-sm font-semibold text-slate-500 leading-tight">{stat.title}</p>
                  <p className="mt-1 sm:mt-2 text-lg sm:text-2xl md:text-3xl font-black text-slate-900 leading-none">{stat.value}</p>
                </div>
                <div className={`p-1.5 sm:p-2.5 rounded-lg sm:rounded-xl ${stat.bg} ${stat.color} shrink-0`}>
                  <stat.icon size={14} className="sm:hidden" />
                  <stat.icon size={20} className="hidden sm:block" />
                </div>
              </div>
              <p className="mt-2.5 sm:mt-4 text-[9px] sm:text-xs font-semibold text-slate-400 leading-tight">{stat.subtext}</p>
            </div>
          ))}
        </div>
            ...
            <div className="relative pt-4 bg-slate-50/50 rounded-2xl border border-slate-100 p-4 overflow-x-auto">
              <div className="h-60 min-w-[600px] flex items-end relative">
                ...
                <svg viewBox="0 0 700 220" className="w-full h-full z-10 overflow-visible">
                  <defs>
                    <linearGradient id="tealGradDash" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <path d={areaD} fill="url(#tealGradDash)" />
        ...
        <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-200">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link href="/admin/analytics" className="text-sm font-semibold text-teal-600 hover:text-teal-700 flex items-center gap-1">
